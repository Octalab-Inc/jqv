...
15 語、選択肢は 0〜5 の score 型、1.7B、生の確率）。比較として通常のチャット生成（thinking OFF / ON）も先頭の語で試した。

| 設定 | 1.7B 正解数 | 備考 |
|---|---:|---|
| 直接: 質問のみ（state 空） | 2/15 | strawberry → 「1」に p=0.98。間違いに高い確信 |
| 直接: state に 1 文字ずつ綴った語を置く | 8/15 | strawberry は「2」（p=1.00）のまま |
| 直接: 綴り + 位置番号を置く | 6/15 | 位置情報はむしろ邪魔 |
| 分解: 綴った state を共有し、各位置に「この位置は 'r' か」の yes/no を分岐させ、外で数える | 3/15 | 1.7B は位置の参照ができず、先頭一致以降に yes が続く |
| 参考: 通常生成 thinking OFF | strawberry は 2（誤り）、119 token | 綴りを書いて数えるが数え間違える |
| 参考: 通常生成 thinking ON | strawberry は 3（正解）、871 token | 位置を一つずつ確認して正答 |
