...
- v1 の scenario を残すか（混合して 2,000 問のままか、v2 を足して増やすか）。既定は v1 を残して v2 を足し、train を 2,000 → 3,000 問に増やす。
- 混合比の temporal_numeric の重み（0.25 のままか）。
- 14B の学習を Mac で回すか GB10 で回すか（GB10 なら vLLM コンテナを止めたままにする必要がある）。
